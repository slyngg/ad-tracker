...
export default function PageShell({ title, subtitle, actions, children, hideHeaderOnMobile, compactMobile, showDatePicker }: PageShellProps) {
  ...
  return (
    <div className={`${compactMobile ? 'p-0 lg:p-6' : 'px-3 py-3 sm:p-4 lg:p-6'} max-w-[1400px] mx-auto`}>
      <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3 sm:mb-6 ${hideHeaderOnMobile ? 'hidden lg:flex' : ''}`}>
        <div className="min-w-0">
          <h1 className="text-lg sm:text-xl font-bold text-ats-text truncate">{title}</h1>
          {showDatePicker ? (
            <div className="flex items-center gap-2 mt-1 sm:mt-0.5">
              <DateRangePicker value={dateRange} onChange={setDateRange} />
              {subtitle && <span className="hidden sm:inline text-sm text-ats-text-muted">{subtitle}</span>}
            </div>
          ) : (
