  ...
  return (
    <PageShell title="Creative Inspo" subtitle="Save and organize competitor ads for inspiration">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex gap-1 bg-ats-card rounded-lg p-1 border border-ats-border">
          <button onClick={() => setTab('saved')} className={`px-4 py-1.5 rounded-md text-sm font-semibold ${tab === 'saved' ? 'bg-ats-accent text-white' : 'text-ats-text-muted hover:text-ats-text'}`}>Saved Library</button>
          <button onClick={() => setTab('following')} className={`px-4 py-1.5 rounded-md text-sm font-semibold ${tab === 'following' ? 'bg-ats-accent text-white' : 'text-ats-text-muted hover:text-ats-text'}`}>Following</button>
        </div>
        {tab === 'saved' ? (
          <button onClick={() => setShowSaveModal(true)} className="px-3 py-1.5 bg-ats-accent text-white rounded-lg text-sm font-semibold">Save New</button>
        ) : (
          <button onClick={() => setShowFollowModal(true)} className="px-3 py-1.5 bg-ats-accent text-white rounded-lg text-sm font-semibold">Follow Brand</button>
        )}
      </div>
        ...
        <>
          <div className="flex gap-2 mb-4">
            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search saved ads..."
              className="bg-ats-card border border-ats-border rounded-lg px-3 py-1.5 text-sm text-ats-text flex-1" />
            <select value={brandFilter} onChange={e => setBrandFilter(e.target.value)}
              className="bg-ats-card border border-ats-border rounded-lg px-3 py-1.5 text-sm text-ats-text">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {filteredSaved.map(s => (
              <div key={s.id} className={cardCls}>
                {s.thumbnail_url && (
                  <img src={s.thumbnail_url} alt="" className="w-full h-32 object-cover rounded-lg mb-3" />
                )}
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-purple-400">{s.brand_name}</span>
                  <span className="text-[10px] text-ats-text-muted">{s.platform}</span>
                </div>
                {s.headline && <div className="text-sm font-semibold text-ats-text mb-1">{s.headline}</div>}
                {s.ad_copy && <div className="text-xs text-ats-text-muted line-clamp-3 mb-2">{s.ad_copy}</div>}
                {s.notes && <div className="text-xs text-amber-400/70 italic mb-2">{s.notes}</div>}
                <div className="flex items-center justify-between mt-2">
                  <span className="text-[10px] text-ats-text-muted">{new Date(s.saved_at).toLocaleDateString()}</span>
            ...
            {filteredSaved.length === 0 && (
              <div className={`${cardCls} col-span-full text-center py-8 text-ats-text-muted`}>
                No saved creatives yet. Click "Save New" to add competitor ads.
              </div>
            )}
          </div>
        </>
      ...
      {!loading && tab === 'following' && (
        <div className="grid gap-3">
          {brands.map(b => (
            <div key={b.id} className={`${cardCls} flex items-center justify-between`}>
              <div>
                <span className="text-sm font-semibold text-ats-text">{b.brand_name}</span>
                <span className="text-xs text-ats-text-muted ml-2">{b.platform}</span>
                {b.platform_page_id && <span className="text-xs text-ats-text-muted ml-2">ID: {b.platform_page_id}</span>}
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-ats-text-muted">Since {new Date(b.followed_at).toLocaleDateString()}</span>
                <button onClick={() => handleUnfollow(b.id)} className="text-xs text-red-400 hover:text-red-300">Unfollow</button>
              </div>
            </div>
          ))}
          {brands.length === 0 && (
            <div className={`${cardCls} text-center py-8 text-ats-text-muted`}>
              Not following any brands yet. Click "Follow Brand" to start tracking competitors.
            </div>
          )}
        </div>
      )}
