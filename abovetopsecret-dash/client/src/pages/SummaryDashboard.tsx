  ...
  return (
      ...
      actions={
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={!summary}
            className="hidden md:flex items-center gap-1.5 px-3 py-2.5 min-h-[44px] bg-ats-card border border-ats-border text-ats-text-muted rounded-lg text-sm hover:text-ats-text hover:border-ats-accent transition-colors disabled:opacity-40"
            title="Export CSV"
          >
            <Download size={14} />
            <span>Export</span>
          ...
          <button
            ...
            className="hidden md:flex items-center gap-1.5 px-3 py-2.5 min-h-[44px] bg-ats-card border border-ats-border text-ats-text-muted rounded-lg text-sm hover:text-ats-text hover:border-ats-accent transition-colors"
            title="Share"
          >
            <Share2 size={14} />
            <span>Share</span>
    ...
    >
      ...
      <div className="md:hidden space-y-3">
        {/* Mobile KPIs — clean 2x2 grid */}
        {(summary || !dateRange.isToday) && (
          <div className="grid grid-cols-2 gap-2">
            <div className={cardCls}>
              <div className="text-[10px] text-ats-text-muted uppercase tracking-widest font-mono">Spend</div>
              <div className="text-lg font-bold text-ats-text font-mono mt-0.5"><AnimatedNumber value={kpi.spend} format={fmt.currency} /></div>
            </div>
            <div className={cardCls}>
              <div className="text-[10px] text-ats-text-muted uppercase tracking-widest font-mono">Revenue</div>
              <div className="text-lg font-bold text-ats-green font-mono mt-0.5"><AnimatedNumber value={kpi.revenue} format={fmt.currency} /></div>
            </div>
            <div className={cardCls}>
              <div className="text-[10px] text-ats-text-muted uppercase tracking-widest font-mono">ROAS</div>
              <div className={`text-lg font-bold font-mono mt-0.5 ${roiColor}`}><AnimatedNumber value={kpi.roas} format={fmt.ratio} /></div>
            </div>
            <div className={cardCls}>
              <div className="text-[10px] text-ats-text-muted uppercase tracking-widest font-mono">Profit</div>
              <div className={`text-lg font-bold font-mono mt-0.5 ${profitColor}`}><AnimatedNumber value={profit} format={fmt.currency} /></div>
            </div>
          </div>
        )}
        ...
        {/* Mobile Chart */}
        <div className={cardCls}>
          <h2 className="text-sm font-semibold text-ats-text mb-2">Spend vs Revenue</h2>
          {tsLoading ? (
            <div className="h-[180px] flex items-center justify-center"><div className="animate-pulse text-ats-text-muted text-sm">Loading...</div></div>
          ) : (
        ...
        <div className="grid grid-cols-3 gap-2">
          {WORKSPACES.map((ws) => (
            <button key={ws.path} onClick={() => navigate(ws.path)} className={`${cardCls} text-center hover:border-ats-accent transition-colors`}>
              <div className="text-xl mb-1">{ws.icon}</div>
              <div className="text-xs font-semibold text-ats-text">{ws.label}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="hidden md:block">
          ...
          <div className="flex gap-2 overflow-x-auto mb-4 pb-1">
            {pinnedMetrics.map(pm => (
              <div key={pm.id} className="bg-ats-accent/10 border border-ats-accent/30 rounded-lg px-4 py-2 min-w-[120px] flex-shrink-0 relative group">
                ...
                <div className="text-lg font-bold text-ats-text font-mono truncate">{pm.format(pm.value)}</div>
              </div>
            ))}
          </div>
        )}
        ...
        {/* KPI Cards */}
        {(summary || !dateRange.isToday) && (
          <div data-tour="summary-cards" className="grid grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
            <div className={cardCls}>
              ...
              <div className="text-2xl font-bold text-ats-text font-mono truncate"><AnimatedNumber value={kpi.spend} format={fmt.currency} /></div>
            ...
            <div className={cardCls}>
              ...
              <div className="text-2xl font-bold text-ats-green font-mono truncate"><AnimatedNumber value={kpi.revenue} format={fmt.currency} /></div>
            ...
            <div className={cardCls}>
              ...
              <div className={`text-2xl font-bold font-mono truncate ${roiColor}`}><AnimatedNumber value={kpi.roas} format={fmt.ratio} /></div>
            </div>
            <div className={cardCls}>
              <div className="text-[11px] text-ats-text-muted uppercase tracking-widest font-mono mb-1">Conversions</div>
              <div className="text-2xl font-bold text-ats-text font-mono truncate"><AnimatedNumber value={kpi.conversions} format={fmt.num} /></div>
            ...
            <div className={cardCls}>
              ...
              <div className={`text-2xl font-bold font-mono truncate ${profitColor}`}><AnimatedNumber value={profit} format={fmt.currency} /></div>
            </div>
          </div>
        )}
