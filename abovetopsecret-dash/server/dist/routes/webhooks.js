...
router.post('/checkout-champ', webhook_verify_1.verifyCheckoutChamp, async (req, res) => {
    try {
        const body = req.body;
        ...
        await db_1.default.query(`INSERT INTO cc_orders_today (order_id, offer_name, revenue, subtotal, tax_amount, order_status, new_customer, utm_campaign, fbclid, subscription_id, quantity, is_core_sku, source)
       ...
       ON CONFLICT (order_id) DO UPDATE SET
         ...
         quantity = EXCLUDED.quantity`, [orderId, offerName, total, subtotal, taxAmount, orderStatus, newCustomer, utmCampaign, fbclid, subscriptionId, quantity, isCoreSku]);
        ...
        if (Array.isArray(upsells)) {
           ...
           VALUES ($1, $2, $3, $4)`, [orderId, upsell.offered ?? true, upsell.accepted ?? false, upsell.offer_name || offerName]);
            }
        }
...
router.post('/shopify', webhook_verify_1.verifyShopify, async (req, res) => {
    try {
        const body = req.body;
        ...
        const quantity = lineItems.reduce((sum, item) => sum + (item.quantity || 1), 0);
        await db_1.default.query(`INSERT INTO cc_orders_today (order_id, offer_name, revenue, subtotal, tax_amount, order_status, new_customer, utm_campaign, fbclid, subscription_id, quantity, is_core_sku, source)
       ...
       ON CONFLICT (order_id) DO UPDATE SET
         ...
         order_status = EXCLUDED.order_status,
         new_customer = EXCLUDED.new_customer,
         quantity = EXCLUDED.quantity`, [orderId, offerName, totalPrice, subtotalPrice, totalTax, orderStatus, newCustomer, utmCampaign, fbclid, quantity]);
        res.json({ success: true, order_id: orderId });
    }
    catch (err) {
