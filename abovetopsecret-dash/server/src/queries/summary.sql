...
SELECT
  ...
  (SELECT COALESCE(SUM(COALESCE(subtotal, revenue)), 0) FROM cc_orders_today WHERE order_status = 'completed' AND (user_id = $1 OR ($1 IS NULL AND user_id IS NULL))) AS total_revenue,
  (SELECT COUNT(DISTINCT order_id) FROM cc_orders_today WHERE order_status = 'completed' AND (user_id = $1 OR ($1 IS NULL AND user_id IS NULL))) AS total_conversions;
